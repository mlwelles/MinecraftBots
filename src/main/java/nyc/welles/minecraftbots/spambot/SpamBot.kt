package nyc.welles.minecraftbots.spambot
...
class SpamBot : CommandExecutor {
    ...
    override fun onCommand(commandSender: CommandSender, command: Command, s: String, args: Array<String>): Boolean {
        fun usage(): Boolean {
            commandSender.sendMessage("[SPAMBOT] Please specify option \"start\" or \"stop\"")
            return false
        }
        if (!commandSender.hasPermission("mcbots.spambot.use")) {
        ...
        when (cmd) {
            "start" -> {
                if (runnable != null) {
                    commandSender.sendMessage("[SPAMBOT] A spammer has already been spawned")
                    return false
                }
                runnable = object : BukkitRunnable() {
                    override fun run() {
                        val message = SpamBot.quotes[(0..SpamBot.quotes.size).random()]
                        Bukkit.getServer().broadcastMessage(message)
                    }
                }
                (runnable as BukkitRunnable).runTaskTimerAsynchronously(MinecraftBots.getInstance()!!, 0L, 5L)
                return true
            }
            "stop" -> {
                if (runnable == null) {
                    commandSender.sendMessage("[SPAMBOT]§c No spammer is running!")
                }
                runnable?.cancel()
                commandSender.sendMessage("[SPAMBOT] Stopped spamming chat")
                runnable = null
                return true
            }
        }
    ...
    companion object {
        val quotes = listOf("A can of ASPARAGUS, 73 pigeons, some LIVE ammo, and a FROZEN DAQUIRI!!",
                ...
                "Didn't KIRKEGAARD wear out his TIRES in VIENNA during a SNOWSTORM of",
                ...
                "Fell th' WHIRLING BUFFERS buffing away all that stress...",
                ...
                "\"THE LITTLE PINK FLESH SISTERS,\" I saw them at th' FLUROESCENT BULB",
                " MAKERS CONVENTION...",
